...
int insere_elemento(Lista *li, double elem){
    ...
    if(lista_vazia(*li) == 0) // Se a lista não estiver vazia
        (*li)->ant = N;       // faz o antecessor do primeiro nó apontar para o novo nó
...
int remove_elemento(Lista *li, double elem){
    ...
    if(aux->valor != elem)  return -1; // Elemento não encontrado
    ...
    else // se aux->ant == NULL, então aux == *li
        *li = aux->prox;
